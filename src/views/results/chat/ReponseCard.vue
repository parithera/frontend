    ...
    <div v-else class="flex flex-col items-center gap-2">
        ...
        <template v-else>
            ...
            <div v-if="response.text != ''" class="self-start">
                <ScrollArea>
                    <div class="overflow-x-scroll pl-10 pb-3 markdown" v-html="markdown.render(response.text)">
                    </div>
                    <ScrollBar orientation="horizontal" />
                </ScrollArea>
                <div v-if="!response.text.includes('Hi, how can I help you today?')" class="flex justify-between px-4">
            ...
            <ScrollArea class="w-full" v-if="response.code != '' && code_visible">
                <div v-html="markdown.render('```python' + response.code + '```')">
                </div>
                <ScrollBar orientation="horizontal" />
            </ScrollArea>
            <Button v-if="response.code != '' && code_visible" @click="code_visible = !code_visible" variant="link">
            ...
            <div v-if="response.followup.length" class="flex flex-col w-full pl-10 pr-4">
                ...
                <Separator></Separator>
                <div v-for="followup in response.followup" :key="followup" class="flex flex-col gap-2 pt-2">
                    <div @click="askFollowup(followup)" class="cursor-pointer">{{ followup }}</div>
                    <Separator></Separator>
                </div>
            </div>
        </template>
